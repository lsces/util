	...
	if ($handle = @opendir($listdirectory)) {
		...
		while ($file = readdir($handle)) {
			...
			if ($TargetObjectType == "dir") {
				...
				$getID3->setOption(["option_md5_data" => isset($_REQUEST["ShowMD5"])]);
		...
		foreach ($DirectoryContents as $dirname => $val) {
			...
			echo "<th>Title</th>";
			if (isset($_REQUEST["ShowMD5"])) {
				echo '<th>MD5&nbsp;File (File) (<a href="'.$_SERVER["SCRIPT_NAME"]."?listdirectory=".rawurlencode($_REQUEST["listdirectory"] ?? ".").'">disable</a>)</th>';
				echo '<th>MD5&nbsp;Data (File) (<a href="'.$_SERVER["SCRIPT_NAME"]."?listdirectory=".rawurlencode($_REQUEST["listdirectory"] ?? ".").'">disable</a>)</th>';
				echo '<th>MD5&nbsp;Data (Source) (<a href="'.$_SERVER["SCRIPT_NAME"]."?listdirectory=".rawurlencode($_REQUEST["listdirectory"] ?? ".").'">disable</a>)</th>';
			} else {
				echo '<th colspan="3">MD5&nbsp;Data (<a href="'.$_SERVER["SCRIPT_NAME"]."?listdirectory=".rawurlencode($_REQUEST["listdirectory"] ?? ".").'&ShowMD5=1">enable</a>)</th>';
			}
			echo "<th>Tags</th>";
			...
			if (isset($DirectoryContents[$dirname]["known"]) && is_array($DirectoryContents[$dirname]["known"])) {
				...
				foreach ($DirectoryContents[$dirname]["known"] as $filename => $fileinfo) {
					...
					if (isset($_REQUEST["ShowMD5"])) {
						echo '<td align="left"><tt>'.($fileinfo["md5_file"]        ?? "&nbsp;")."</tt></td>";
						echo '<td align="left"><tt>'.($fileinfo["md5_data"]        ?? "&nbsp;")."</tt></td>";
						echo '<td align="left"><tt>'.($fileinfo["md5_data_source"] ?? "&nbsp;")."</tt></td>";
					} else {
