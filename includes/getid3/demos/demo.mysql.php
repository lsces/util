...
function RemoveAccents($string) {
	...
	return strtr(strtr($string, "������������������������������������������������������������", "SZszYAAAAAACEEEEIIIINOOOOOOUUUUYaaaaaaceeeeiiiinoooooouuuuyy"), ["�" => "TH", "�" => "th", "�" => "DH", "�" => "dh", "�" => "ss", "�" => "OE", "�" => "oe", "�" => "AE", "�" => "ae", "�" => "u"]);
}
...
function BitrateColor($bitrate, $BitrateMaxScale=768) {
	...
	$Bcomponent = ( $bitrate > 127 ) ? max( ( 255 - $bitrate ) * 2, 0 ) : max( $bitrate * 2, 0 );
...
function AcceptableExtensions( $fileformat, $audio_dataformat = "", $video_dataformat = "" )
{
	...
	if (!empty( $video_dataformat )) {
		return ( $AcceptableExtensionsVideo[$fileformat][$video_dataformat] ?? [] );
	}

		return ( $AcceptableExtensionsAudio[$fileformat][$audio_dataformat] ?? [] );
...
$IgnoreNoTagFormats = [ "", "png", "jpg", "gif", "bmp", "swf", "zip", "mid", "mod", "xm", "it", "s3m" ];
...
if (!empty( $_REQUEST["scan"] ) || !empty( $_REQUEST["newscan"] ) || !empty( $_REQUEST["rescanerrors"] )) {
	...
	elseif (!empty( $_REQUEST["scan"] ) || !empty( $_REQUEST["newscan"] )) {
		...
		$DirectoriesToScan = [ @$_REQUEST["scan"] ? $_REQUEST["scan"] : $_REQUEST["newscan"] ];
		$DirectoriesScanned = [];
				...
				foreach ( $FieldsToCompare as $fieldname ) {
					...
					if ($tagtype == "id3v1") {
						...
						elseif ($fieldname == "track") {
							...
							if ((int) ( @$Comments[$tagtype][$fieldname][0] ) != (int) ( @$Comments["all"][$fieldname][0] )) {
								$tagvalues .= $fieldname . " = [[" . @$Comments[$tagtype][$fieldname][0] . "]]" . "\n";
								$Mismatched[$tagtype] = true;
								$FileOK = false;
							}
	...
	if (!empty( $_REQUEST["m3u"] )) {
		...
		while ( ( $row = mysql_fetch_array( $result ) ) && ( $row["num"] > 1 ) ) {
			...
			while ( $row2 = mysql_fetch_array( $result2 ) ) {
				...
				$titles[] = $row2["title"];
				$remixes[] = $row2["remix"];
				$bitrates[] = ( $row2["vbr_method"] ) ? "<B" . ( $row2["encoder_options"] ? ' style="text-decoration: underline; cursor: help;" TITLE="' . $row2["encoder_options"] : "" ) . '">' . BitrateText( $row2["audio_bitrate"] / 1000 ) . "</b>" : BitrateText( $row2["audio_bitrate"] / 1000 );
